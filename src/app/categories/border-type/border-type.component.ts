...
export class BorderTypeComponent implements OnInit, OnDestroy {
  ...
  helpChart(res: any, chart: any, chartDiv: string) {
    ...
    chart.exporting.filePrefix =
      this.lang === 'EN'
        ? 'Number of visits By Border Types'
        : 'ვიზიტების რაოდენობა საზღვრის ტიპების მიხედვით';
  ...
  createSeries(field: string, name: string, chart: any, ragac: string) {
    ...
    if (this.lang == 'GEO') {
      if (this.tType == 1) {
        bullet.tooltipText =
          '[bold]{name}[/] საზღვრით ვიზიტების რაოდენობა, საწყის პერიოდთან შედარებით,\n{year} წელს შეიცვალა [bold]{valueY.formatNumber("#.%")}-ით';
      } else {
        bullet.tooltipText =
          '[bold]{name}[/] საზღვრით ვიზიტების რაოდენობა, საწყის პერიოდთან შედარებით,\n{year} წელს შეიცვალა [bold]{valueY.formatNumber("#.%")}-ით';
      }
    ...
    if (this.lang == 'ENG') {
      if (this.tType == 1) {
        bullet.tooltipText =
          'Number of visits by [bold]{name}[/] border, compared to the initial period,\nin {year} year has changed by [bold]{valueY.formatNumber("#.%")}';
      } else {
        bullet.tooltipText =
          'Number of visits by [bold]{name}[/] border, compared to the initial period,\nin {year} year has changed by [bold]{valueY.formatNumber("#.%")}';
      }
